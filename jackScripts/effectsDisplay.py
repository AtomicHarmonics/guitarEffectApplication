...
def updateDisplay(device, inputMessage):
    font_path = str(Path(__file__).resolve().parent.joinpath('fonts', 'ProggyTiny.ttf'))
    font2 = ImageFont.truetype(font_path, 16)
    y = inputMessage
    ...
    with canvas(device) as draw:
        draw.text((0, 0), firstLine, font=font2, fill="white")
        if device.height >= 32:
            draw.text((0, 8),  zeroLine, font=font2, fill="white")
            draw.text((0, 16), secondLine, font=font2, fill="white")
            draw.text((0, 24), thirdLine, font=font2, fill="white")
            draw.text((0, 32), fourthLine, font=font2, fill="white")
            draw.text((0, 40), fifthLine, font=font2, fill="white")
            draw.text((0, 48), sixthLine, font=font2, fill="white")
            draw.text((0, 56), seventhLine, font=font2, fill="white")
            draw.text((0, 64), eighthLine, font=font2, fill="white")
...
#python3 effectsDisplay.py --display pygame --width 240 --height 320
